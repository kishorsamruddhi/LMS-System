...
import { getAdmin_view_report_by_learner_id } from '../../Api/get';
...
const _View_Learner_Report = () => {
  ...
  const allModules = data?.courses?.length > 0 ? data?.courses.flatMap((cor) => cor.modules).length : 0;

  const progressInPercentage = (data?.completedModules?.length * 100 /
    allModules).toFixed(0);
  ...
  return (
    <div className='viewReport'>
      <LinkBackButton path={"/dashboard/Training/learners"} />
      <div>
